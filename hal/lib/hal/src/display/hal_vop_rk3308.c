...
#if defined(HAL_VOP_MODULE_ENABLED) && defined(SOC_RK3308)
...
static inline uint16_t VOP_CalScaleFactor(int src, int dst, int shift);
...
{
    if ((winId != VOP_WIN0) && (winId != VOP_WIN1)) {
        return false;
    } else {
        return true;
    }
...
static inline eVOP_ScaleMode VOP_GetSclMode(int src, int dst)
{
    if (src < dst) {
        return SCALE_UP;
    } else if (src > dst) {
...
static inline uint16_t VOP_SclGetBiliDnVskip(int srcH, int dstH, int vSkipLines)
{
    int actHeight;

    actHeight = (srcH + vSkipLines - 1) / vSkipLines;

    return GET_SCL_FT_BILI_DN(actHeight, dstH);
}

static inline int VOP_SclGetVSkipLines(uint32_t srcH, uint32_t dstH)
{
    uint32_t vSkipLines;
...
static uint16_t VOP_CalScale(eVOP_ScaleMode mode, uint32_t src, uint32_t dst,
                             bool isHorizontal, int vsuMode, int *vSkipLines)
{
    ...
    if (isHorizontal) {
        ...
        if (mode == SCALE_UP) {
            ...
            if (vSkipLines) {
                *vSkipLines = VOP_SclGetVSkipLines(src, dst);
                val = VOP_SclGetBiliDnVskip(src, dst,
                                            *vSkipLines);
            } else {
                val = GET_SCL_FT_BILI_DN(src, dst);
            }
                            ...
                            struct DISPLAY_MODE_INFO *pModeInfo)
{
    ...
    int hSub = VOP_GetHorSubSampling(pWinState->hwFormat);
    int vSub = VOP_GetVerSubSampling(pWinState->hwFormat);
    int vSkipLines = 0;
    uint16_t scalX, scalY;
    uint16_t cbcrSrcW = pWinState->srcW / hSub;
    uint16_t cbcrSrcH = pWinState->srcH / vSub;
    eVOP_ScaleMode yrgbHorSclMode, yrgbVerSclMode;
    eVOP_ScaleMode cbcrHorSclMode = SCALE_NONE;
    eVOP_ScaleMode cbcrVerSclMode = SCALE_NONE;
    ...
    if (!pWinState->winEn) {
        if (pWinState->winId == VOP_WIN0) {
            VOP_MaskWrite(&s_vopRegMir.WIN0_CTRL0,
                          &pReg->WIN0_CTRL0,
                          VOP_WIN0_CTRL0_WIN0_EN_SHIFT,
                          VOP_WIN0_CTRL0_WIN0_EN_MASK, pWinState->winEn);
        } else {
            VOP_MaskWrite(&s_vopRegMir.WIN1_CTRL0,
                          &pReg->WIN1_CTRL0,
    ...
    if ((pWinState->winId == VOP_WIN1) && (pWinState->hwFormat > VOP_FMT_RGB565)) {
        HAL_DBG_ERR("Unsupport YUV format in win%d\n", pWinState->winId);
    ...
    if (pWinState->winId == VOP_WIN0) {
        VOP_Write(&s_vopRegMir.WIN0_VIR,
                  &pReg->WIN0_VIR, cbcrStride << 16 | yStride);
        VOP_Write(&s_vopRegMir.WIN0_YRGB_MST0,
                  &pReg->WIN0_YRGB_MST0, pWinState->yrgbAddr);
        VOP_Write(&s_vopRegMir.WIN0_CBR_MST0,
                  &pReg->WIN0_CBR_MST0, pWinState->cbcrAddr);
    } else {
        VOP_Write(&s_vopRegMir.WIN1_VIR,
                  &pReg->WIN1_VIR, yStride);
        VOP_Write(&s_vopRegMir.WIN1_MST,
                  &pReg->WIN1_MST, pWinState->yrgbAddr);
    }

    if (pWinState->winId == VOP_WIN0) {
        if (IS_YUV_FORMAT(pWinState->hwFormat)) {
            cbcrHorSclMode = VOP_GetSclMode(cbcrSrcW, pWinState->crtcW);
            cbcrVerSclMode = VOP_GetSclMode(cbcrSrcH, pWinState->crtcH);

            scalX = VOP_CalScale(cbcrHorSclMode, cbcrSrcW, pWinState->crtcW,
                                 true, 0, NULL);
            scalY = VOP_CalScale(cbcrVerSclMode, cbcrSrcH, pWinState->crtcH,
                                 false, 0, &vSkipLines);
            VOP_Write(&s_vopRegMir.WIN0_SCL_FACTOR_CBR,
                      &pReg->WIN0_SCL_FACTOR_CBR, scalX | scalY << 16);
        } else {
            yrgbHorSclMode = VOP_GetSclMode(pWinState->srcW, pWinState->crtcW);
            yrgbVerSclMode = VOP_GetSclMode(pWinState->srcH, pWinState->crtcH);

            scalX = VOP_CalScale(yrgbHorSclMode, pWinState->srcW, pWinState->crtcW,
                                 true, 0, NULL);
            scalY = VOP_CalScale(yrgbVerSclMode, pWinState->srcH, pWinState->crtcH,
                                 false, 0, &vSkipLines);
            VOP_Write(&s_vopRegMir.WIN0_SCL_FACTOR_YRGB,
                      &pReg->WIN0_SCL_FACTOR_YRGB, scalX | scalY << 16);
        }
    }

    if (pWinState->winId == VOP_WIN0) {
        VOP_Write(&s_vopRegMir.WIN0_ACT_INFO,
                  &pReg->WIN0_ACT_INFO, actInfo);
        VOP_Write(&s_vopRegMir.WIN0_DSP_INFO,
                  &pReg->WIN0_DSP_INFO, dspInfo);
        VOP_Write(&s_vopRegMir.WIN0_DSP_ST,
                  &pReg->WIN0_DSP_ST, dspSt);
    } else {
        VOP_Write(&s_vopRegMir.WIN1_DSP_INFO,
                  &pReg->WIN1_DSP_INFO, dspInfo);
        VOP_Write(&s_vopRegMir.WIN1_DSP_ST,
                  &pReg->WIN1_DSP_ST, dspSt);
    }
    ...
    if (pWinState->winId == VOP_WIN0) {
        ...
        VOP_Write(&s_vopRegMir.WIN0_ALPHA_CTRL,
                  &pReg->WIN0_ALPHA_CTRL, val);
        VOP_Write(&s_vopRegMir.WIN0_COLOR_KEY,
                  &pReg->WIN0_COLOR_KEY, pWinState->colorKey);
    } else {
        ...
        VOP_Write(&s_vopRegMir.WIN1_ALPHA_CTRL,
                  &pReg->WIN1_ALPHA_CTRL, val);
        VOP_Write(&s_vopRegMir.WIN1_COLOR_KEY,
                  &pReg->WIN1_COLOR_KEY, pWinState->colorKey);
    }

    if (pWinState->zpos == 0 || pWinState->zpos == 1) {
        if (pWinState->winId == VOP_WIN0) {
            VOP_MaskWrite(&s_vopRegMir.DSP_CTRL2,
                          &pReg->DSP_CTRL2,
                          VOP_DSP_CTRL2_DSP_WIN0_TOP_SHIFT,
                          VOP_DSP_CTRL2_DSP_WIN0_TOP_MASK, pWinState->zpos);
        } else {
            VOP_MaskWrite(&s_vopRegMir.DSP_CTRL2,
                          &pReg->DSP_CTRL2,
                          VOP_DSP_CTRL2_DSP_WIN0_TOP_SHIFT,
...
#endif /* HAL_VOP_MODULE_ENABLED && SOC_RK3308 */
