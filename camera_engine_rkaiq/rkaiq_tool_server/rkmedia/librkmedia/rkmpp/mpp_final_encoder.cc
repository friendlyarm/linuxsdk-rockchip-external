...
#define MPP_MAX_BPS_VALUE 100000000 // 100Mb
    ...
    bool MPPMJPEGConfig::InitConfig(MPPEncoder& mpp_enc, MediaConfig& cfg)
    {
        ...
        LOG("MPP Encoder[JPEG]: w x h(%d[%d] x %d[%d]), qfactor:%d\n", img_info.width, line_size, img_info.height, img_info.vir_height, img_cfg.qfactor);
    ...
    bool MPPCommonConfig::InitConfig(MPPEncoder& mpp_enc, MediaConfig& cfg)
    {
        ...
        if (vconfig.rc_mode && vconfig.rc_quality && (!strcmp(vconfig.rc_mode, KEY_VBR)) && (!vconfig.qp_max || !vconfig.qp_min))
        {
            VideoEncoderQp qps;
        ...
        {
            ENCODER_CFG_CHECK(vconfig.qp_init, vconfig.qp_min, vconfig.qp_max, (vconfig.qp_min + vconfig.qp_max) / 2, "qpInit");
        }
        ENCODER_CFG_CHECK(vconfig.qp_step, 1, (vconfig.qp_max - vconfig.qp_min), VALUE_MIN((vconfig.qp_max - vconfig.qp_min), 2), "qpStep");
        ...
        ENCODER_CFG_CHECK(vconfig.qp_min_i, 1, VALUE_MIN(vconfig.qp_max_i, 48), VALUE_MIN(vconfig.qp_max_i, 8), "qpMini");
        ...
        if ((image_info.pix_fmt == PIX_FMT_FBC0) || (image_info.pix_fmt == PIX_FMT_FBC2))
        {
            if ((rotation == MPP_ENC_ROT_90) || (rotation == MPP_ENC_ROT_270))
            {
                ...
                LOG("MPP Encoder: rotation in fbc mode, Resolution "
                    ...
                    image_info.height, image_info.vir_height, image_info.width, image_info.vir_width, image_info.width, image_info.vir_width, image_info.height, image_info.vir_height);
            }
            rotation = MPP_ENC_ROT_0;
        }
        ...
        LOG("MPP Encoder: bps:[%d,%d,%d] fps: [%d/%d]->[%d/%d], gop:%d\n", bps_max, bps_target, bps_min, fps_in_num, fps_in_den, fps_out_num, fps_out_den, gop);
        ...
        switch (code_type)
        {
            case MPP_VIDEO_CodingAVC:
                ...
                ret |= mpp_enc_cfg_set_s32(enc_cfg, "h264:trans8x8", (vconfig.trans_8x8 && (vconfig.profile == 100)) ? 1 : 0);
        ...
        LOG("MPP Encoder: w x h(%d[%d] x %d[%d])\n", image_info.width, line_size, image_info.height, image_info.vir_height);
    ...
    bool MPPCommonConfig::CheckConfigChange(MPPEncoder& mpp_enc, uint32_t change, std::shared_ptr<ParameterBuffer> val)
    {
        ...
        else if (change & VideoEncoder::kBitRateChange)
        {
            ...
            if (bps_max && bps_min && bps_target)
            {
                if ((bps_max > MPP_MAX_BPS_VALUE) || (bps_max < MPP_MIN_BPS_VALUE) || (bps_min > MPP_MAX_BPS_VALUE) || (bps_min < MPP_MIN_BPS_VALUE) || (bps_target > bps_max) || (bps_target < bps_min))
                {
                    ret = -1;
                }
            }
        ...
        else if (change & VideoEncoder::kRcQualityChange)
        {
            ...
            LOG("MPP Encoder: [%s:%s->%s] init:%d, setp:%d, min:%d, "
                "max:%d, min_i:%d, max_i:%d\n",
                vconfig.rc_mode, vconfig.rc_quality, rc_quality, qps.qp_init, qps.qp_step, qps.qp_min, qps.qp_max, qps.qp_min_i, qps.qp_max_i);
        ...
        else if (change & VideoEncoder::kQPChange)
        {
            ...
            LOG("MPP Encoder: new qp:[%d, %d, %d, %d, %d, %d]\n", qps->qp_init, qps->qp_step, qps->qp_min, qps->qp_max, qps->qp_min_i, qps->qp_max_i);
