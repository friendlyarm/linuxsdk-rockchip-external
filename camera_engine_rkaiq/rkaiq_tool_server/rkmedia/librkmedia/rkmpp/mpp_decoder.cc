...
namespace easymedia
{
    ...
    MPPDecoder::MPPDecoder(const char* param) : output_format(PIX_FMT_NONE), fg_limit_num(kFRAMEGROUP_MAX_FRAMES), need_split(1), timeout(MPP_POLL_NON_BLOCK), coding_type(MPP_VIDEO_CodingUnused), support_sync(true), support_async(true)
    {
    ...
    int MPPDecoder::Process(const std::shared_ptr<MediaBuffer>& input, std::shared_ptr<MediaBuffer>& output, std::shared_ptr<MediaBuffer> extra_output _UNUSED)
    {
        if (!support_sync)
        {
            errno = ENOSYS;
        ...
        if (output_buffer_size == 0 || !output->IsHwBuffer())
        {
            ...
            if (coding_type == MPP_VIDEO_CodingMJPEG && buffer)
            {
                ...
                output_buffer_size = CalPixFmtSize(fmt, UPALIGNTO16(w), UPALIGNTO16(h)) + UPALIGNTO16(w) * UPALIGNTO16(h) / 2;
            }
            else
            {
    ...
    RETRY_GET_FRAME:
        ...
        if (mpp_frame_get_info_change(mppframe))
        {
            ...
            LOG("MppDec Info change get, %dx%d in (%dx%d)\n", img_info.width, img_info.height, img_info.vir_width, img_info.vir_height);
