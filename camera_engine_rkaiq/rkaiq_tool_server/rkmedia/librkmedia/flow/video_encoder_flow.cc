...
namespace easymedia
{
    ...
    bool encode(Flow* f, MediaBufferVector& input_vector)
    {
        ...
        if (vf->md_flow)
        {
            ...
            else
            {
                ...
                {
    #ifndef NDEBUG
                    LOGD("VEnc Flow: get md info(cnt=%d): %p, %zuBytes\n", md_info->GetValidSize() / sizeof(INFO_LIST), md_info.get(), md_info->GetValidSize());
    #endif
                    if (md_info->GetSize() >= sizeof(INFO_LIST))
                    {
                        ...
                        while (info->flag)
                        {
                            LOGD("VEnc Flow: mdinfo: flag:%d, upleft:<%d, %d>, downright:<%d, "
                                 "%d>\n",
                                 info->flag, info->up_left[0], info->up_left[1], info->down_right[0], info->down_right[1]);
                            info += 1;
                        }
    #endif
        ...
        if (!roi_region_str.empty())
        {
            ...
            if (roi_regions_cnt)
            {
                ...
                for (int i = 0; i < roi_regions_cnt; i++)
                {
                    ...
                    LOG("VEnc Flow: Roi Regions[%d]: (%d,%d,%d,%d,%d,%d,%d,%d,%d)\n", i, roi_regions[i].x, roi_regions[i].y, roi_regions[i].w, roi_regions[i].h, roi_regions[i].intra, roi_regions[i].quality, roi_regions[i].qp_area_idx, roi_regions[i].area_map_en, roi_regions[i].abs_qp_en);
                }
    ...
    void VideoEncoderFlow::Dump(std::string& dump_info)
    {
        ...
        if (mc.type == Type::Image)
        {
            ...
            sprintf(str_line, "  Input: %d(%d)x%d(%d) fmt:%s\r\n", mc.img_cfg.image_info.width, mc.img_cfg.image_info.vir_width, mc.img_cfg.image_info.height, mc.img_cfg.image_info.vir_height, PixFmtToString(mc.img_cfg.image_info.pix_fmt));
            dump_info.append(str_line);
            memset(str_line, 0, sizeof(str_line));
        ...
        else if (mc.type == Type::Video)
        {
            ...
            sprintf(str_line, "  Input: %d(%d)x%d(%d) fmt:%s\r\n", imgcfg.image_info.width, imgcfg.image_info.vir_width, imgcfg.image_info.height, imgcfg.image_info.vir_height, PixFmtToString(imgcfg.image_info.pix_fmt));
            dump_info.append(str_line);
            memset(str_line, 0, sizeof(str_line));
