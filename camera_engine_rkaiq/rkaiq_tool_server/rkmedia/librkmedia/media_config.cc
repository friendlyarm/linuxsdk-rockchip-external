...
namespace easymedia
{
    ...
    std::string get_video_encoder_config_string(const ImageInfo& info, const VideoEncoderCfg& cfg)
    {
        ...
        if (cfg.rc_quality && strcmp(cfg.rc_quality, KEY_HIGHEST) && strcmp(cfg.rc_quality, KEY_HIGHER) && strcmp(cfg.rc_quality, KEY_HIGH) && strcmp(cfg.rc_quality, KEY_MEDIUM) && strcmp(cfg.rc_quality, KEY_LOW) && strcmp(cfg.rc_quality, KEY_LOWER) && strcmp(cfg.rc_quality, KEY_LOWEST))
        {
    ...
    int video_encoder_set_rc_quality(std::shared_ptr<Flow>& enc_flow, const char* rc_quality)
    {
        ...
        if (strcmp(rc_quality, KEY_HIGHEST) && strcmp(rc_quality, KEY_HIGHER) && strcmp(rc_quality, KEY_HIGH) && strcmp(rc_quality, KEY_MEDIUM) && strcmp(rc_quality, KEY_LOW) && strcmp(rc_quality, KEY_LOWER) && strcmp(rc_quality, KEY_LOWEST))
        {
    ...
    int video_encoder_set_qp(std::shared_ptr<Flow>& enc_flow, VideoEncoderQp& qps)
    {
        ...
        // qp_max       - 8 ~ 51
        // qp_min       - 0 ~ 48
        if ((qps.qp_max && ((qps.qp_max > 51) || (qps.qp_max < 8))) || (qps.qp_max_i && ((qps.qp_max_i > 51) || (qps.qp_max_i < 8))) || (qps.qp_min < 0) || (qps.qp_min > 48) || (qps.qp_min_i < 0) || (qps.qp_min_i > 48) || (qps.qp_min > qps.qp_max) || (qps.qp_min_i > qps.qp_max_i))
        {
