...
bool CamCaptureHelper::init(MetaInfo* meta)
{
    ...
    ALOGD("camera capture init start: dev %s, wh %dx%d, format %d, bufcnt %d", meta->video_dev, meta->width, meta->height, meta->format, mPixBufCnt);
    ...
    for (i = 0; i < mPixBufCnt; i++)
    {
        ...
        if (V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE == buf.type)
        {
            ...
            mCambuf[i].start = mmap(NULL /* start anywhere */, buf.m.planes[0].length, PROT_READ | PROT_WRITE /* required */, MAP_SHARED /* recommended */, mDevfd, buf.m.planes[0].m.mem_offset);
        }
        else
        {
            buf_len = buf.length;
            mCambuf[i].start = mmap(NULL /* start anywhere */, buf.length, PROT_READ | PROT_WRITE /* required */, MAP_SHARED /* recommended */, mDevfd, buf.m.offset);
        }
