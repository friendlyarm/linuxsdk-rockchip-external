...
rtl8168_set_mac_mcu_8168fp_1(struct rtl_hw *hw)
{
        ...
        if (hw->HwPkgDet == 0x00 || hw->HwPkgDet == 0x0F) {
                ...
                for (i = 0; i < ARRAY_SIZE(mcu_patch_code_8168fp_1_1); i++) {
                        rtl8168_mac_ocp_write(hw, 0xF800 + i * 2, mcu_patch_code_8168fp_1_1[i]);
                }

                rtl8168_mac_ocp_write(hw, 0xFC26, 0x8000);

                rtl8168_mac_ocp_write(hw, 0xFC28, 0x0890);
                rtl8168_mac_ocp_write(hw, 0xFC2A, 0x0712);
                rtl8168_mac_ocp_write(hw, 0xFC2C, 0x0974);
                rtl8168_mac_ocp_write(hw, 0xFC2E, 0x09FC);
                rtl8168_mac_ocp_write(hw, 0xFC30, 0x0A0E);
                rtl8168_mac_ocp_write(hw, 0xFC32, 0x0A56);
                rtl8168_mac_ocp_write(hw, 0xFC34, 0x0A68);
                rtl8168_mac_ocp_write(hw, 0xFC36, 0x0A84);
        ...
        rtl8168_mac_ocp_write(hw, 0xFC38, breakPointEnabled);
}
...
rtl8168_set_mac_mcu_8168fp_8116as_2(struct rtl_hw *hw)
{
        ...
        for (i = 0; i < ARRAY_SIZE(mcu_patch_code_8168fp_8116as_2); i++) {
                rtl8168_mac_ocp_write(hw, 0xF800 + i * 2, mcu_patch_code_8168fp_8116as_2[i]);
        }

        rtl8168_mac_ocp_write(hw, 0xFC26, 0x8000);

        rtl8168_mac_ocp_write(hw, 0xFC28, 0x2AAC);
        rtl8168_mac_ocp_write(hw, 0xFC2A, 0x0B14);
        rtl8168_mac_ocp_write(hw, 0xFC2C, 0x0BE4);

        rtl8168_mac_ocp_write(hw, 0xFC38, 0x0007);
}
...
_rtl8168_set_mac_mcu_8168fp_2(struct rtl_hw *hw)
{
        ...
        for (i = 0; i < ARRAY_SIZE(mcu_patch_code_8168fp_2); i++) {
                rtl8168_mac_ocp_write(hw, 0xF800 + i * 2, mcu_patch_code_8168fp_2[i]);
        }
        ...
        rtl8168_mac_ocp_write(hw, 0xFC2A, 0x0B14);
        rtl8168_mac_ocp_write(hw, 0xFC2C, 0x0BE4);
        rtl8168_mac_ocp_write(hw, 0xFC2E, 0x2A5C);
        //rtl8168_mac_ocp_write(hw, 0xFC30, 0x07B0);

        if (true == rtl8168_check_dash_other_fun_present(hw))
                rtl8168_mac_ocp_write(hw, 0xFC38, 0x0006);
        else
                rtl8168_mac_ocp_write(hw, 0xFC38, 0x000E);
}
...
rtl8168_set_mac_mcu_8168fp_3(struct rtl_hw *hw)
{
        ...
        for (i = 0; i < ARRAY_SIZE(mcu_patch_code_8168fp_3); i++) {
                rtl8168_mac_ocp_write(hw, 0xF800 + i * 2, mcu_patch_code_8168fp_3[i]);
        }

        rtl8168_mac_ocp_write(hw, 0xFC26, 0x8000);

        rtl8168_mac_ocp_write(hw, 0xFC28, 0x2B14);
        //rtl8168_mac_ocp_write(hw, 0xFC2A, 0x07B0);

        if (true == rtl8168_check_dash_other_fun_present(hw))
                rtl8168_mac_ocp_write(hw, 0xFC38, 0x0000);
        else
                rtl8168_mac_ocp_write(hw, 0xFC38, 0x0001);
}
//------------------------------------phy 8111FP---------------------------------------
