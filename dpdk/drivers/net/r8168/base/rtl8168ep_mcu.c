...
rtl8168_set_mac_mcu_8168ep_1(struct rtl_hw *hw)
{
        ...
        for (i = 0; i < ARRAY_SIZE(mcu_patch_code_8168ep_1); i++) {
                rtl8168_mac_ocp_write(hw, 0xF800 + i * 2, mcu_patch_code_8168ep_1[i]);
        }

        rtl8168_mac_ocp_write(hw, 0xFC26, 0x8000);

        rtl8168_mac_ocp_write(hw, 0xFC28, 0x2549);
        rtl8168_mac_ocp_write(hw, 0xFC2A, 0x06A5);
        rtl8168_mac_ocp_write(hw, 0xFC2C, 0x0763);
}
...
rtl8168_set_mac_mcu_8168ep_2(struct rtl_hw *hw)
{
        ...
        for (i = 0; i < ARRAY_SIZE(mcu_patch_code_8168ep_2); i++) {
                rtl8168_mac_ocp_write(hw, 0xF800 + i * 2, mcu_patch_code_8168ep_2[i]);
        }

        rtl8168_mac_ocp_write(hw, 0xFC26, 0x8000);

        rtl8168_mac_ocp_write(hw, 0xFC28, 0x0BB3);
        if (false == rtl8168_check_dash_other_fun_present(hw))
                rtl8168_mac_ocp_write(hw, 0xFC2A, 0x1FC7);
        //rtl8168_mac_ocp_write(hw, 0xFC2C, 0x0485);
}
...
rtl8168_set_phy_mcu_8168ep_2(struct rtl_hw *hw)
{
        ...
        rtl8168_mdio_write(hw, 0x14, 0x8700);
        rtl8168_mdio_write(hw, 0x13, 0xB82E);
        rtl8168_mdio_write(hw, 0x14, 0x0001);

        rtl8168_mdio_write(hw, 0x1F, 0x0A43);

        rtl8168_mdio_write(hw, 0x13, 0x83DD);
        rtl8168_mdio_write(hw, 0x14, 0xAF83);
        rtl8168_mdio_write(hw, 0x14, 0xE9AF);
        rtl8168_mdio_write(hw, 0x14, 0x83EE);
        rtl8168_mdio_write(hw, 0x14, 0xAF83);
        rtl8168_mdio_write(hw, 0x14, 0xF1A1);
        rtl8168_mdio_write(hw, 0x14, 0x83F4);
        rtl8168_mdio_write(hw, 0x14, 0xD149);
        rtl8168_mdio_write(hw, 0x14, 0xAF06);
        rtl8168_mdio_write(hw, 0x14, 0x47AF);
        rtl8168_mdio_write(hw, 0x14, 0x0000);
        rtl8168_mdio_write(hw, 0x14, 0xAF00);
        rtl8168_mdio_write(hw, 0x14, 0x00AF);
        rtl8168_mdio_write(hw, 0x14, 0x0000);

        rtl8168_mdio_write(hw, 0x13, 0xB818);
        rtl8168_mdio_write(hw, 0x14, 0x0645);
